...
import { bootstrapDocMarker, getDefaultOptions } from "doc-marker";
...
bootstrapDocMarker({
  ...
  formRenderersImporter: async () => {
    ...
    const resqRenderers = (await import("./formRenderersAndCells"))
      .formRenderers;
    return [...dmRenderers, ...resqRenderers];
  },
  formCellsImporter: async () => {
    const dmCells = await defaultOptions.formCellsImporter();
    const resqCells = (await import("./formRenderersAndCells")).formCells;
