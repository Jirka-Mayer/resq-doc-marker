...
export function UploadDialog() {
  ...
  const { dmOptions, filesDatabase } = useContext(DocMarkerContext);

  const [isOpen, setIsOpen] = useAtom<boolean>(isOpenAtom);
  const [isLoading, setIsLoading] = useState<boolean>(true);

  const fileRef = useRef<AppFile | null>(null);
  ...
  async function handleDialogOpenning() {
    ...
    const fileBody = file.toJson();
    setFileName(file.constructFileName());
    setFileAlreadyUploaded(typeof fileBody["uploadedAt"] === "string");
    setCaseId(fileBody["resqCaseId"] || null);
    setRecordId(fileBody["resqRecordId"] || null);
  ...
  async function performUploading() {
    ...
    try {
      const givenFileJson = fileRef.current!.toJson();
      ...
      setRecordId(modifiedFileJson!["resqRecordId"] || null);
      setUploadDone(true);
    } catch (e) {
      const err = e as Error;
                ...
                >
                  ...
                  >
                    ...
                    {Object.keys(providers).map((pi) => (
                      <MenuItem key={pi} value={String(pi)}>
                        {providers[pi]}
                      </MenuItem>
                    ))}
                  </Select>
